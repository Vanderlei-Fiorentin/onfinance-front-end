...
@Component({
  selector: 'app-alteracao-lancamento',
  templateUrl: './alteracao-lancamento.component.html',
  styleUrls: ['./alteracao-lancamento.component.css']
})
export class AlteracaoLancamentoComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.params.subscribe(params => {
      this.findLanctoById(params['id']);
      this.findEventosLanctoEntradaById(params['id']);
      this.findProdutosLanctoSaidaById(params['id']);
      this.findSaidasLanctoEntradaById(params['id']);
    });
    this.findEmpresas();
    this.findContas();
    this.findCartoes();
  ...
  update() {
    ...
    this.lanctoContabilService.updateLanctoContabilGeral(formulario).subscribe({
      next: (lancto) => {
        this.alertService.success('Dados alterados com sucesso!', this.options);
        this.router.navigate(['lanctos']);
      },
      error: (error) => {
  ...
  findLanctoById(id: number) {
    this.lanctoContabilService.findById(id).subscribe({
      next: (lancto) => {
        this.lancamento = LanctoContabilModel.builder(lancto);
        this.onInitForms();
      },
      error: (error) => {
